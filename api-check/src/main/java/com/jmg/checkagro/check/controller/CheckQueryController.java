...
public class CheckQueryController {
    ...
    public List<CheckResponse> findByCustomer(@PathVariable String typeDocument, @PathVariable String valueDocument) {
        return checkRepository.findByDocumentTypeCustomerAndDocumentValueCustomer(
                typeDocument, valueDocument).stream().map(checkMapper::toCheckResponse).toList();
    }
    ...
    public List<CheckResponse> findByProvider(@PathVariable String typeDocument, @PathVariable String valueDocument) {
        return checkRepository.findByDocumentTypeProviderAndDocumentValueProvider(
                typeDocument, valueDocument).stream().map(checkMapper::toCheckResponse).toList();
    }

    @GetMapping("/customer/resume/{typeDocument}/{valueDocument}")
    public CustomerCheckLimitResponse customerResume(@PathVariable String typeDocument,
                                                     @PathVariable String valueDocument) throws CheckException {
